    ...
    requestJSON(requri, function(json) {
      if(json.message == "Not Found" || username == '') {
        ...
        var outhtml = `<h2>${fullname} 
          <span class="smallname">(@<a href="${profileurl}" target="_blank">${username}</a>)</span>
        </h2>
        <div class="ghcontent">
          <div class="float-left img-fluid img-thumbnail m-1">
            <a href="${profileurl}" target="_blank">
              <img src="${aviurl}" width="80" height="80" alt="${username}">
            </a>
          </div>
          <p>Followers: ${followersnum} - Following: ${followingnum}<br>Repos: ${reposnum}</p></div>
